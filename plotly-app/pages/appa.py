...
threshold = {
    "NO2": {"very poor": 400, "poor": 200, "moderate": 100, "decent": 40},
    "PM10": {"very poor": 100, "poor": 50, "moderate": 35, "decent": 20},
    "PM2.5": {"very poor": 50, "poor": 25, "moderate": 18, "decent": 10},
    "O3": {"very poor": 240, "poor": 180, "moderate": 120, "decent": 89},
    "CO": {"very poor": 20, "poor": 10, "moderate": 7.5, "decent": 5.0},
    "SO2": {"very poor": 500, "poor": 350, "moderate": 200, "decent": 100},
}
...
stations = list(
    load_data_from_psql("select distinct stazione from appa_data;").stazione
)
...
def get_all_data() -> pd.DataFrame:
    ...
    df = load_data_from_psql(
        "select stazione, inquinante, ts, valore from appa_data where date_part('year', ts) >= date_part('year', CURRENT_DATE) - 10"
    )
...
def plot_compare_years(start_date, end_date, selected_appa_station, pollutant):
    s = start_date.split("-")
    e = end_date.split("-")
    q = querys.query_appa_compare_years(
        s_day=s[2], s_month=s[1], e_day=e[2], e_month=e[1]
    )
    ...
    fig = px.line(
        ...
        y="Value",
        color="Year",
        category_orders={"Year": df["Year"].sort_values().unique()},
    )
    fig.update_layout(
    ...
    fig.add_hrect(
        y0=0,
        y1=threshold[pollutant]["decent"],
        line_width=0,
        fillcolor="lightblue",
        opacity=0.1,
    )
    fig.add_hrect(
        y0=threshold[pollutant]["decent"],
        y1=threshold[pollutant]["moderate"],
        line_width=0,
        fillcolor="green",
        opacity=0.1,
    ) if df["Value"].max() > threshold[pollutant]["decent"] else None
    fig.add_hrect(
        y0=threshold[pollutant]["moderate"],
        y1=threshold[pollutant]["poor"],
        line_width=0,
        fillcolor="yellow",
        opacity=0.1,
    ) if df["Value"].max() > threshold[pollutant]["moderate"] else None
    fig.add_hrect(
        y0=threshold[pollutant]["poor"],
        y1=threshold[pollutant]["very poor"],
        line_width=0,
        fillcolor="red",
        opacity=0.1,
    ) if df["Value"].max() > threshold[pollutant]["poor"] else None
    fig.add_hrect(
        y0=threshold[pollutant]["very poor"],
        y1=1000,
        line_width=0,
        fillcolor="#660033",
        opacity=0.1,
    ) if df["Value"].max() > threshold[pollutant]["very poor"] else None
...
def line_plot(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            showlegend=False,
            hovertemplate="%{x}<br>%{y} μg/m3 <extra></extra>"
            if not pollutant == "CO"
            else "%{x}<br>%{y} ppm <extra></extra>",
        )
    )
...
def get_pollutants(selected_appa_station: str) -> dbc.RadioItems:
    ...
    df = load_data_from_psql(
        f"select distinct inquinante as Pollutant from appa_data where stazione = '{selected_appa_station}';"
    )
...
def update_main_plot(
    ...
    selected_appa_period: str,
    selected_weekday_period,
    btn_search,
    check_years,
):
    """
    ...
    if (
        (
            ...
            or (
                "selected-pollutant" == callback_context.triggered_id
                and LAST_CLICKED == "btn_search_date"
            )
            or (
                "selected-weekday-period" == callback_context.triggered_id
                and LAST_CLICKED == "btn_search_date"
            )
        )
        and start_date
    ...
    if check_years:
        fig_main = plot_compare_years(
            start_date, end_date, selected_appa_station, selected_pollutant
        )
    else:
        fig_main = line_plot(
            data,
    ...
    fig_year.update_xaxes(title="Months", title_font_size=12)
    ...
    fig_week.update_traces(
        opacity=0.6, marker_line_width=1.5, marker_line_color="black"
    )
    # -------------------------------------------------------------
                ...
                dbc.CardBody(
                    [
                        ...
                        html.Div(
                            [
                                html.H3(
                                    f"{limit_pollutants[pollutant]}",
                                    style={"color": "red"},
                                ),
                                html.Div(
                                    [
                                        html.P(
                                            f"In {y}, the average daily limit was exceeded ",
...
toast = dbc.Toast(
    [
        ...
        search,
        dcc.Checklist(
            options=[" Show comparison over years"],
            id="check_years",
...
gas_btns = html.Div(
    dbc.RadioItems(id="selected-pollutant"),
    id="appa-pollutants",
...
header = html.Div(
    [title, dropdownWrapper, download_btn, download_it, gas_btns],
